...
visited = [False for x in range(number_of_edges)]
...
def if_changed(x, distances):
    for distance in distances[x - 1]:
        if edges[distance[0]] >= distance[1] + edges[x]:

            edges[distance[0]] = distance[1] + edges[x]

            if_changed(distance[0], distances)
...
def Dijkstra(distances):
    ...
    for x in range(len(distances)):
        for distance in distances[x]:
            if edges[x] > distance[1] + edges[distance[0]]:
                if distance[1] + edges[x] == 48:
                    print(distance[1] + edges[x])
                edges[x] = distance[1] + edges[distance[0]]
                if_changed(x, distances)        
    print(edges)
